...
kal_int32 PMIC_IMM_GetCurrent(void)
{
	...
	*/
	...
	pmic_turn_on_clock(TURN_OFF);//mtk  20150812  add
...
kal_uint32 PMIC_IMM_GetOneChannelValue(mt6350_adc_ch_list_enum dwChannel, int deCount, int trimd)
{
	...
	*/
	...
	wake_lock(&pmicAuxadc_irq_lock);
	mutex_lock(&pmic_adc_mutex);//mtk  20150812  add
	...
	default:
		...
		wake_unlock(&pmicAuxadc_irq_lock);
		mutex_unlock(&pmic_adc_mutex);//mtk  20150812  add
	return -1;
	break;
	}
