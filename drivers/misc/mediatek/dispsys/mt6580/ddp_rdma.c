...
void rdma_set_ultra(unsigned int idx, unsigned int width, unsigned int height, unsigned int bpp, unsigned int frame_rate,void * handle, enum RDMA_MODE mode, bool isidle)
{
  ...
  if (idx == 0)
  {
      ...
      else if ( (width >= 720) || ((width>=1280) && (height<=720)) ) // HD
      {
          ...
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x0B01194B);
          // Max ultra
          //DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x0C011880);
          if (mode == RDMA_MODE_DIRECT_LINK)
          {
              sodi_threshold = 101|(112<<10);//low:101	high:112 (direct link in HD)	
              fifo_valid_size = 100 + 1;
          }
          else // mode == RDMA_MODE_DECOUPLE
          {
              sodi_threshold = 101|(112<<10);//low:101 high:112 (decouple in HD)
              fifo_valid_size = 100 + 1;
